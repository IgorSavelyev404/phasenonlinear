...
root = tk.Tk()
root.title('Построение фазовых траекторий')
...
def phase_trace(e):
    ...
    #else:
    integrate.main(phase_trace = True)
...
def transition_process(e):
    ...
    try:
        ...
        integrate.main(transition=True)
...
def set_input_matr(e):
    ...
    if hyst == 'С запаздыванием':
        sign=SignHystClassSystem.SignHyst(line_up=l1, line_down=l2, up = m1, down = m2, last_x = last_x, last_t = start_time, state = vkl,zapazdivanie = True)
        equa.sign_hyst.append(sign.get_response)
            
    elif hyst == 'С опережением':
        sign=SignHystClassSystem.SignHyst(line_up=l1, line_down=l2, up = m1, down = m2, last_x =last_x, last_t = start_time, state = vk, zapazdivanie = False)
        equa.sign_hyst.append(sign.get_response)
...
def phase_plane(e):
    ...
    try:
        ...
        integrate.main(phase_plane = True)
