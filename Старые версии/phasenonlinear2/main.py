...
def makeform_for_system(root, fields):
    ...
    row.pack(side=tk.TOP,
             ...
             pady=5)
    row= tk.Frame(root)
    lab_l1 = tk.Label(row, width=5, text="l1", anchor='w')
    ...
    lab_l2 = tk.Label(row, width=5, text="l2", anchor='w')
    ...
    lab_m1 = tk.Label(row, width=5, text="m1", anchor='w')
    ...
    lab_m2 = tk.Label(row, width=5, text="m2", anchor='w')
    ...
    # конец расчета
    lab_end = tk.Label(row, width=10, text='Конец расчета:', anchor='w')
    ...
    row.pack(side=tk.TOP,
             ...
             pady=5)
    row=  tk.Frame(root) 
    lab_type = tk.Label(row, width=20, text='Тип гистерезиса:', anchor='w')
...
def set_input_matr(e):
    ...
    if hyst == 'С запаздыванием':
        for i in range(equa.size):
            sign=SignHystClass1.SignHyst(line_up=l1, line_down=l2, up = m1, down = m2, last_x =equa.ic[i], last_t = start_time)
